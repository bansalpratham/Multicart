...
for (let i = 0; i < allOrdersData.length; i++) {
  ...
  if (!found) {
    vendorOrderGraph.push({ vendor: vendorName, orders: 1 });
  }
}
    ...
    return (
      <div key={i} className='bg-white/5 border border-white/10 rounded-xl p-4'>
          ...
          <p className='text-xs text-gray-400 mb-2'>
            Status: <span className={`capitalize ${vendor.verificationStatus === "approved" ? "text-gray-400" : "text-yellow-400"}`}>
...
<p className="text-green-400 font-semibold">
  ...
  {/* Bar Graph */}
  <div className='bg-white/5 border border-white/10 rounded-xl p-4 h-70 sm:h-122.5'>

...
function VendorApproval() {
  ...
  const pendingVendors = Array.isArray(allVendorsData)?allVendorsData.filter((v)=>v.verificationStatus === "pending"): []
  ...
  return (
   ...
   <div key={index} className='bg-white/10 border border-white/20 rounded-xl p-4 space-y-2'>
      ...
      <button className='w-full mt-3 bg-blue-600 hover:bg-blue-700 text-sm py-2 rounded-lg' onClick={()=>setSelectedVendor(vendor)}>
              Check Details
      </button>
   </div>
            ))
      ...
      {selectedVendor && (
          ...
          <button disabled={loading} onClick={handleApproved} className='flex-1 bg-green-600 hover:bg-green-700 py-2 rounded-lg text-sm '>
            ...
            Reject</button>
            <button  onClick={()=>setSelectedVendor(null)} className='flex-1 bg-gray-700 hover:bg-green-700 py-2 rounded-lg text-sm '>
            Cancel</button>
          </div>

...
function VendorOrders() {
  ...
  const [otpModel , setOtpModel] = useState<any|null>()
  ...
  const orders = Array.isArray(allOrdersData) ?
    allOrdersData.filter((o) => {
        const vendorId = typeof o.productVendor === 'object' ? o.productVendor?._id : o.productVendor;
        return String(vendorId) === String(userData?._id);
    }) : []
  ...
  return (
    <div className='w-full px-3 sm:px-6 lg:px-10 py-6 text-white'>
      <div className='flex justify-between'>
        <h1 className='text-xl sm:text-2xl lg:text-3xl font-bold mb-6 text-center sm:text-left'>Vendor Orders</h1>
  ...
  <span className="text-orange-400 font-semibold capitalize">
    Returned
                   ...
                   { order.orderStatus !== "cancelled" && order.orderStatus !== "delivered" && order.orderStatus !== "returned" &&
                    ...
                    value={order.orderStatus} className='bg-white/10 w-full border border-white/20 rounded px-2 py-1'>
        ...
        {orders.length === 0 ? (
          ...
          orders.map((order, index) => (
            <div key={index} className='bg-white/10 border border-white/20 rounded-xl p-4 space-y-2'>
              ...
              <p className='text-sm'><b>Buyer:</b> {order.address?.name}</p>
              <div className="mt-2 text-sm">
                {order.products.map((p: any, i: number) => (
                  <p key={i}>{p.product?.title || "Deleted Product"} x {p.quantity}</p>
                ))}
              </div>
              <div className="mt-3 text-sm">
                <b>Status:</b> <span className="capitalize">{order.orderStatus}</span>
              </div>
  ...
  <span className="text-orange-400 font-semibold capitalize">
    Returned
             ...
             { order.orderStatus !== "cancelled" && order.orderStatus !== "delivered" && order.orderStatus !== "returned" && 
              ...
              value={order.orderStatus} className='bg-white/10 w-full border border-white/20 rounded px-2 py-1 mt-2'>
                {statusOptions.map((s, i) => (
                  <option key={i} value={s} className='bg-black'>{s}</option>
                ))}
              </select>}

...
function VendorProducts() {
  ...
  const myProducts = currentUser?._id && allProductsData.length ? allProductsData.filter((p:any)=>p.vendor === currentUser?._id || p.vendor?._id === currentUser?._id) : []

  const toggleIsActive = async (productId:string , currentisActive:boolean ) => {
    try {
      
      const result = await axios.post("/api/vendor/isActive" , {
  ...
  myProducts.map((p, index) => (
    <React.Fragment key={index}>
      ...
      <tr className='border-t border-white/10 hover:bg-white/5'>
        ...
        <td className='p-4'>
          <span className={`px-3 py-1 rounded-full text-xs bg-gray-500/30 ${
        ...
        <td className='p-4 text-center'>
          <span className={`text-sm ${p.isActive ? "text-green-400" : "text-red-400"}`}>
            {p?.isActive ? "Active" : "InAction"}
          </span>
        </td>
   ...
   <div key={index} className='bg-white/10 border border-white/20 rounded-xl p-4 space-y-2'>
      <div className='flex items-center gap-3'>
            ...
            whileTap={{scale:0.97}}
             onClick={()=>toggleIsActive(String(p._id) , Boolean(p.isActive) )}
            disabled = {p.verificationStatus !== "approved"}
            className={`px-3 py-1 rounded text-sm ${p.verificationStatus ===  "approved" ? "bg-green-600 hover:bg-gray-700" : "bg-gray-600 cursor-not-allowed"}`}
            >
              {p.isActive ? "Disable" : "Enable"}
            </motion.button>
              </div>

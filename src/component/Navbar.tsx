...
import { AiOutlineSearch, AiOutlineUser, AiOutlineShoppingCart, AiOutlineMenu, AiOutlineClose, AiOutlineHome, AiOutlineAppstore, AiOutlinePhone, AiOutlineShop, AiOutlineLogin, AiOutlineLogout, AiOutlineSolution } from 'react-icons/ai'
            ...
            <div className='max-w-7xl mx-auto px-6 py-3 flex justify-between items-center '>
                ...
                <div className='hidden md:flex items-center gap-6'>
                    ...
                    <div className='relative'>
                        {user?.image ? <Image src={user?.image} alt='user' width={40} height={40} className='w-10 h-10 rounded-full object-cover border border-gray-700 cursor-pointer' onClick={() => setOpenMenu(!openMenu)} /> : <IconBtn Icon={AiOutlineUser} onClick={() => setOpenMenu(!openMenu)} />}
                ...
                <div className='md:hidden flex items-center gap-4'>
                    ...
                    {user?.role == "vendor" || user?.role == "admin" ? (
                        <>
                            <IconBtn Icon={AiOutlinePhone} onClick={() => router.push("/support")} />
                            <div className='relative'>
                                {user?.image ? <Image src={user?.image} alt='user' width={32} height={32} className='w-8 h-8 rounded-full object-cover border border-gray-700 cursor-pointer' onClick={() => setOpenMenu(!openMenu)} /> : <IconBtn Icon={AiOutlineUser} onClick={() => setOpenMenu(!openMenu)} />}
                            ...
                            <AnimatePresence>
                                ...
                                >
                                    ...
                                    <div className='flex flex-col gap-4 text-lg'>
                                        ...
                                        <SidebarBtnForSignOut label="SignOut" Icon={AiOutlineLogout} path={"/login"} setSidebarOpen={setSidebarOpen} />
                                    </div>
...
const CartBtn = ({ router, count }: any) => {
    return <motion.button whileHover={{ scale: 1.1 }} onClick={() => router.push("/cart")} className='relative'  >
...
const SidebarBtn = ({ label, path, router, Icon, setSidebarOpen }: any) => {
    return <button className='flex items-center gap-3 px-4 py-2 rounded-lg bg-[#6a69693c] hover:bg-white/10 text-left ' onClick={() => { router.push(path); setSidebarOpen(false) }} >
        <Icon size={20} /> {label}
    </button>
}

const SidebarBtnForSignOut = ({ label,Icon, setSidebarOpen }: any) => {
    return <button className='flex items-center gap-3 px-4 py-2 rounded-lg bg-[#6a69693c] hover:bg-white/10 text-left ' onClick={() => { signOut() ; setSidebarOpen(false) }} >
        <Icon size={20} /> {label}
    </button>
}
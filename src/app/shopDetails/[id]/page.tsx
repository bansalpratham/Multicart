...
function page() {
  ...
  ? allProductsData.filter(
      (p: any) =>
        String(typeof p.vendor === "object" ? p.vendor._id : p.vendor) ===
        String(vendor._id)
    )
  : []
   ...
   <div className='min-h-screen bg-linear-to-br from-gray-900 via-black to-gray-900 text-white p-6'>
    ...
    >
        <div className='relative w-full h-60 overflow-hidden rounded-xl bg-black flex items-center justify-center '>
            {vendor.image ? <Image src={vendor.image} alt='img'
            fill className='object-cover'/> : <span className='text-white'>No image
            found</span>}
        </div>
    ...
    <h2 className='text-2xl font-bold mb-8'>Product By : {vendor?.
    shopName}</h2>

    ...
    <div className='min-h-screen bg-linear-to-br from-gray-900 via-black to-gray-900 text-white px-4 pt-20 pb-10'>
      ...
      className='max-w-3xl mx-auto bg-white/10 backdrop-blur-xl p-6 sm:p-10 rounded-2xl border border-white/20 shadow-xl'>
            ...
            <div className='grid grid-cols-1 sm:grid-cols-2 gap-4'>
                    ...
                    type="text" className='focus:outline-none focus:ring-2 focus:ring-blue-500  p-3 bg-white/10 border border-white/20 rounded' />
                     <input
                ...
                className='p-3 bg-white/10 border border-white/20 rounded text-white'>
                    ...
                    {categories.map((cat)=>(
                        <option key={cat} className='bg-gray-800' value={cat} >{cat}</option>
                    ))}
                </select>

            </div>
           {category === "Others" && <input type='text' className='focus:outline-none focus:ring-2 focus:ring-blue-500 mt-4 w-full p-3 bg-white/10 border border-white/20 rounded' onChange={(e)=>setCustomCategory(e.target.value)} value={customCategory} />}
           <textarea
            ...
            <div className='flex items-center gap-3 mt-5'>
                        <input type="checkbox" className='w-5 h-5' checked={isWearable} onChange={()=>setIsWearable(prev => !prev)}/>
          ...
          <div className='grid grid-cols-1 sm:grid-cols-2 gap-4 mt-6'>
            <input type="text" className='p-3 bg-white/10 border border-white/20 rounded focus:outline-none focus:ring-2 focus:ring-blue-500 '
                ...
                placeholder='ReplacementDays (e.g. 1 year)'
            />
            <input type="text" className='p-3 bg-white/10 border border-white/20 rounded focus:outline-none focus:ring-2 focus:ring-blue-500'
          ...
          <div className='flex items-center gap-3 mt-5'>
                        <input type="checkbox" className='w-5 h-5' checked={freeDelivery} onChange={()=>setFreeDelivery(prev => !prev)}/>
                        <span className='text-sm'>Free Delivery </span>
                        <input type="checkbox" className='w-5 h-5 ml-4' checked={payOnDelivery} onChange={()=>setPayOnDelivery(prev => !prev)}/>
            ...
            <div className='grid grid-cols-2 sm:grid-cols-4 gap-4'>
                ...
                <div>
                 ...
                 <div>
                    ...
                    />
                    <label
                    className='cursor-pointer bg-gray-800 p-2 rounded h-28 flex items-center justify-center border border-white/2 hover:border-blue-500'
                    htmlFor='img2'>
                        {preview2 ? (
                 ...
                 <div>
                    ...
                    htmlFor='img4'>
                        {preview4 ? (
                            <Image src={preview4} alt='img3' width={120} height={120} className='w-full h-full object-cover rounded' />
                        ) : (
                            <div className='flex flex-col items-center text-gray-400 text-xs'>
                                <FiUpload size={22} />
                                <span>Image 4</span>
            ...
            <div className='mt-6'>
                    ...
                    <div className='flex gap-2'>
                        <input type="text" className='flex-1 p-3 bg-white/10 border border-white/20 rounded focus:outline-none focus:ring-2 focus:ring-blue-500' onChange={(e)=>setCurrentPoint(e.target.value)} value={currentPoint} placeholder={`Point ${pointIndex + 1}`} />
                        <button type='button' className='px-4 bg-blue-600 hover:bg-blue-700 rounded font-semibold' onClick={handleAddPoints} >Add Point</button>
                    </div>

...
const getSuggestions = async ()=>{
  ...
  return (
    <div className='min-h-screen bg-linear-to-br from-gray-950 via-black to-gray-900 p-3 sm:p-6'>
  ...
  <div className='bg-black/50 border border-white/10 rounded-2xl p-4 overflow-y-auto'>
    ...
    {userData?.role !== "admin" && <p className='text-xs text-yellow-50 bg-yellow-800 rounded p-3 my-2 leading-relaxed'>
  {userData?.role === "user" && (
    <>Note: The vendor's response may take 1-2 hours. In some cases, you may receive a reply sooner.</>
  )}
  {userData?.role === "vendor" && (
    <>Note: The admin's response may take 1-2 hours. In some cases, you may receive a reply sooner.</>
  )}
</p>}
    {users?.length === 0 ? (
...
>
  ...
  <div className='px-4 pb-2 flex gap-2 flex-wrap'>
    {suggestions.map((s, i) => (
      <div
        onClick={() => setText(s)}
        key={i}
        className='text-xs px-3 py-1 rounded-full bg-blue-500/10 text-blue-300 border border-blue-500/30 hover:bg-blue-500/20 transition cursor-pointer'
      >
        {s}
      </div>
    ))}
  ...
  <input 
    ...
    className="flex-1 bg-black/80 text-white border border-white/10 rounded-full px-5 py-2.5 outline-none focus:border-blue-500" 
  />
  <button onClick={sendMessage} className="bg-blue-600 hover:bg-blue-700 w-11 h-11 rounded-full flex items-center justify-center">
